fuzzygrep() {
  ...
  local file
  file=$(rg --files-with-matches --no-heading --color=never "$query" | \
         fzf --preview "rg --color=always --line-number '$query' {}" \
  ...
  local selection
  selection=$(rg --line-number --no-heading --color=never "$query" "$file" | \
  ...
  local column
  column=$(awk -v l="$linecontent" -v q="$query" 'BEGIN{print index(l,q)}')
